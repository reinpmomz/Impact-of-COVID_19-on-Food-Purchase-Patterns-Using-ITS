...
## Period All
gls_model_all_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  ...
  ## Model metrics
  model_metrics <- tibble::tibble(item = "nutrient - proximate"
                                  , group = nn
                                  , period = "all"
...
gls_model_interruption1_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
...
gls_model_interruption2_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
...
## ITS plot
df_gls_ITS_model_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_proximate[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_proximate[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_proximate[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_ITS_model_plot_weekly_proximate <- data.table::rbindlist(df_gls_ITS_model_weekly_proximate) %>%
...
ggsave(plot=gls_ITS_model_plot_weekly_proximate, height = 7.5, width = 13.5,
       filename = paste0("gls_ITS_plot_weekly_proximate",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_gls_prediction_model_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_proximate[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_proximate[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_proximate[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
ggsave(plot=gls_prediction_model_plot_weekly_proximate, height = 7.5, width = 13.5,
       filename = paste0("gls_prediction_plot_weekly_proximate",".png"),
       path = output_Dir, bg='white')
