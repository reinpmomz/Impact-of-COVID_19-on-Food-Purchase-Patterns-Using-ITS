...
df_sensitivity_gls_arima_model_interruption1_weekly <- dplyr::bind_rows(data.table::rbindlist(gls_model_interruption1_weekly_nova_coefficients)
  ...
  dplyr::mutate(across(any_of(c("Pr(>|z|)", "Pr(>|t|)")), ~round(.x, 4))
                ...
                , term = ifelse(term == "(Intercept)", "intercept",
                                ifelse(term == "weekly_time", "xreg", term
                                                                   ))
                ) %>%
  dplyr::rename(any_of(c(estimate_gls = "Estimate", pvalue_gls = "Pr(>|z|)", pvalue_gls = "Pr(>|t|)"))
                ) %>%
  dplyr::inner_join(dplyr::bind_rows(data.table::rbindlist(arima_model_interruption1_weekly_nova_coefficients)
  ...
  dplyr::mutate(term = ifelse(term == "intercept", paste0("\U03B2", "\U2080", " (Intercept)"),
                              ifelse(term == "xreg", paste0("\U03B2", "\U2081", " (Pre-Covid)"), term
                                                          ))
...
sensitivity_gls_arima_model_interruption1_weekly_plot <- df_sensitivity_gls_arima_model_interruption1_weekly %>%
...
print(sensitivity_gls_arima_model_interruption1_weekly_plot)

### Saving the sensitivity plot
ggsave(plot=sensitivity_gls_arima_model_interruption1_weekly_plot, height = 7, width = 11,
       filename = paste0("sensitivity_gls_arima_model_interruption1_weekly",".png"),
       path = output_Dir, bg='white')

### Saving the sensitivity descriptive
flextable::save_as_docx(descriptive_table(df = df_sensitivity_gls_arima_model_interruption1_weekly #%>% dplyr::filter(item != "food group")
                                          ,flex_table = TRUE
                                          ,foot_note = "n (%)"
                                          ,caption = "Sensitivity-gls/arima model interruption1"
                                          ,include = c("sensitivity")
                                          ), 
                        path = base::file.path(output_Dir, "descriptive_sensitivity_gls_arima_model_interruption1.docx"),
