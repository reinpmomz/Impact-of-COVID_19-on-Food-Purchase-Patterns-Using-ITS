...
## Period All
arima_model_all_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  
  df <- df_analysis_ITS_weekly_proximate %>%
    dplyr::filter(new_label == nn)
  
  timeseries <- stats::ts(df$value, start = as.numeric(min(df$week_date)) 
  ...
  ljung_test_df <- tibble::tibble(item = "nutrient - proximate"
                                  , group = nn
                                  , period = "all"
  ...
  coefficients_df <- dplyr::bind_cols(lmtest::coeftest(arima_model#, df = arima_model$nobs-1
    ...
    dplyr::mutate(item = "nutrient - proximate"
                  , group = nn) %>%
  ...
  ## Model metrics
  model_metrics <- dplyr::bind_cols(tibble::tibble( item = "nutrient - proximate"
                                                    , group = nn
                                                    , period = "all"
...
arima_model_interruption1_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  ...
  ## Model metrics
  model_metrics <- dplyr::bind_rows(tibble::tibble( item = "nutrient - proximate"
                                                    , group = nn
                                                    , period = "interruption 1"
                                                    , method = arima_forecast[["method"]]
                                                    , loglik = arima_forecast[["model"]][["loglik"]]
...
arima_model_interruption2_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
...
## ITS plot
df_arima_ITS_model_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  
  df <- arima_model_all_weekly_proximate[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(arima_model_interruption1_weekly_proximate[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(arima_model_interruption2_weekly_proximate[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
arima_ITS_model_plot_weekly_proximate <- data.table::rbindlist(df_arima_ITS_model_weekly_proximate) %>%
...
ggsave(plot=arima_ITS_model_plot_weekly_proximate, height = 7.5, width = 13.5,
       filename = paste0("arima_ITS_plot_weekly_proximate",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_arima_prediction_model_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  
  df <- arima_model_all_weekly_proximate[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(arima_model_interruption1_weekly_proximate[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(arima_model_interruption2_weekly_proximate[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
arima_prediction_model_plot_weekly_proximate <- data.table::rbindlist(df_arima_prediction_model_weekly_proximate) %>%
...
ggsave(plot=arima_prediction_model_plot_weekly_proximate, height = 7.5, width = 13.5,
       filename = paste0("arima_prediction_plot_weekly_proximate",".png"),
       path = output_Dir, bg='white')
