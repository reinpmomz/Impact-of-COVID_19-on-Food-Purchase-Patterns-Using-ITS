...
## Period All
gls_model_all_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
  ...
  autocorrel_model <- sapply(1:nrow(autocorrel), function(x) {
    ...
    p <- try(summary(nlme::gls(food_group_prop ~ weekly_time + intervention1 + post_intervention1_time +
  ...
  gls_model <- nlme::gls(food_group_prop ~ weekly_time + intervention1 + post_intervention1_time +
                           intervention2 + post_intervention2_time
                         , data = df
                         , method = "ML" #REML, ML
  ...
  ## Model metrics
  model_metrics <- tibble::tibble( item = "food group"
                                  , group = nn
                                  , period = "all"
                                  ...
                                  , data = "Training set"
                                  #, R2 = caret::R2(gls_forecast, df$food_group_prop) #R2 = cor(df$food_group_prop - gls_forecast)^2
                                  , me = mean((df$food_group_prop - gls_forecast))
                                  #, mse = mean((df$food_group_prop - gls_forecast)^2)
                                  , rmse = caret::RMSE(gls_forecast, df$food_group_prop) #sqr(MSE)
                                  , mae = caret::MAE(gls_forecast, df$food_group_prop) #MAE = mean(abs(df$food_group_prop - gls_forecast))
                                  , mpe = mean((df$food_group_prop - gls_forecast)*100/gls_forecast)
                                  , mape = mean(abs(df$food_group_prop - gls_forecast)*100/gls_forecast)
                                  , mase = Metrics::mase(df$food_group_prop, gls_forecast)
                                  )
...
gls_model_interruption1_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
  ...
  autocorrel_model <- sapply(1:nrow(autocorrel), function(x) {
    ...
    p <- try(summary(nlme::gls(food_group_prop ~ weekly_time
                         , data = df_train
                         , correlation= corARMA(p=autocorrel$pval[i],q=autocorrel$qval[i], form = ~ weekly_time)
                         ,method="ML")
                     )$AIC
  ...
  gls_model <- nlme::gls(food_group_prop ~ weekly_time
                         , data = df_train
                         , method = "ML" #REML, ML
                         , correlation = corARMA(p = pval, q = qval, form = ~ weekly_time, fixed = FALSE)
                         )
  ...
  model_metrics <- tibble::tibble( item = "food group"
                                  ...
                                  , sigma2 = (summary_gls_model[["sigma"]])^2
                                  , data = "Test set"
                                  #, R2 = caret::R2(gls_forecast, df_test$food_group_prop) #R2 = cor(df_test$food_group_prop - gls_forecast)^2
                                  , me = mean((df_test$food_group_prop - gls_forecast))
                                  #, mse = mean((df_test$food_group_prop - gls_forecast)^2)
                                  , rmse = caret::RMSE(gls_forecast, df_test$food_group_prop) #sqr(MSE)
                                  , mae = caret::MAE(gls_forecast, df_test$food_group_prop) #MAE = mean(abs(df_test$food_group_prop - gls_forecast))
                                  , mpe = mean((df_test$food_group_prop - gls_forecast)*100/gls_forecast)
                                  , mape = mean(abs(df_test$food_group_prop - gls_forecast)*100/gls_forecast)
                                  , mase = Metrics::mase(df_test$food_group_prop, gls_forecast)
                                  )
...
gls_model_interruption2_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
...
## ITS plot
df_gls_ITS_model_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_food_group[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_food_group[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_food_group[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_ITS_model_plot_weekly_food_group <- data.table::rbindlist(df_gls_ITS_model_weekly_food_group) %>%
...
ggsave(plot=gls_ITS_model_plot_weekly_food_group, height = 7.5, width = 13.5,
       filename = paste0("gls_ITS_plot_weekly_food_group",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_gls_prediction_model_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_food_group[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_food_group[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_food_group[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_prediction_model_plot_weekly_food_group <- data.table::rbindlist(df_gls_prediction_model_weekly_food_group) %>%
...
ggsave(plot=gls_prediction_model_plot_weekly_food_group, height = 7.5, width = 13.5,
       filename = paste0("gls_prediction_plot_weekly_food_group",".png"),
       path = output_Dir, bg='white')
