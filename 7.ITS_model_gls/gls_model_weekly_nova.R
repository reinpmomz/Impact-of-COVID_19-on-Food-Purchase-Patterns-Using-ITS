...
## Period All
gls_model_all_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  ...
  gls_model <- nlme::gls(nova_prop ~ weekly_time + intervention1 + post_intervention1_time +
                           intervention2 + post_intervention2_time
                         , data = df
                         , method = "ML" #REML, ML
  ...
  ## Model metrics
  model_metrics <- tibble::tibble( item = "nova"
                                  , group = nn
                                  , period = "all"
                                  ...
                                  , data = "Training set"
                                  #, R2 = caret::R2(gls_forecast, df$nova_prop) #R2 = cor(df$nova_prop - gls_forecast)^2
                                  , me = mean((df$nova_prop - gls_forecast))
                                  #, mse = mean((df$nova_prop - gls_forecast)^2)
                                  , rmse = caret::RMSE(gls_forecast, df$nova_prop) #sqr(MSE)
                                  , mae = caret::MAE(gls_forecast, df$nova_prop) #MAE = mean(abs(df$nova_prop - gls_forecast))
                                  , mpe = mean((df$nova_prop - gls_forecast)*100/gls_forecast)
                                  , mape = mean(abs(df$nova_prop - gls_forecast)*100/gls_forecast)
                                  , mase = Metrics::mase(df$nova_prop, gls_forecast)
                                  )
...
gls_model_interruption1_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  ...
  gls_model <- nlme::gls(nova_prop ~ weekly_time
                         , data = df_train
                         , method = "ML" #REML, ML
                         , correlation = corARMA(p = pval, q = qval, form = ~ weekly_time, fixed = FALSE)
                         )
  ...
  model_metrics <- tibble::tibble( item = "nova"
                                  ...
                                  , sigma2 = (summary_gls_model[["sigma"]])^2
                                  , data = "Test set"
                                  #, R2 = caret::R2(gls_forecast, df_test$nova_prop) #R2 = cor(df_test$nova_prop - gls_forecast)^2
                                  , me = mean((df_test$nova_prop - gls_forecast))
                                  #, mse = mean((df_test$nova_prop - gls_forecast)^2)
                                  , rmse = caret::RMSE(gls_forecast, df_test$nova_prop) #sqr(MSE)
                                  , mae = caret::MAE(gls_forecast, df_test$nova_prop) #MAE = mean(abs(df_test$nova_prop - gls_forecast))
                                  , mpe = mean((df_test$nova_prop - gls_forecast)*100/gls_forecast)
                                  , mape = mean(abs(df_test$nova_prop - gls_forecast)*100/gls_forecast)
                                  , mase = Metrics::mase(df_test$nova_prop, gls_forecast)
                                  )
...
gls_model_interruption2_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
...
## ITS plot
df_gls_ITS_model_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_nova[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_nova[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_nova[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_ITS_model_plot_weekly_nova <- data.table::rbindlist(df_gls_ITS_model_weekly_nova) %>%
  ...
  scale_y_continuous( labels = scales::percent, n.breaks = 7, expand = expansion(mult = c(0.05,0.05))
                      ) +
...
ggsave(plot=gls_ITS_model_plot_weekly_nova, height = 7.5, width = 13.5,
       filename = paste0("gls_ITS_plot_weekly_nova",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_gls_prediction_model_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_nova[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_nova[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_nova[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_prediction_model_plot_weekly_nova <- data.table::rbindlist(df_gls_prediction_model_weekly_nova) %>%
...
ggsave(plot=gls_prediction_model_plot_weekly_nova, height = 7.5, width = 13.5,
       filename = paste0("gls_prediction_plot_weekly_nova",".png"),
       path = output_Dir, bg='white')
