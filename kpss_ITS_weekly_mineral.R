...
# Kwiatkowski-Phillips-Schmidt-Shin (KPSS) for level or trend stationarity - Weekly
...
kpss_ITS_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  ...
  kpss_level_test <- tseries::kpss.test(df$value
                                        , null = "Level"
                                        )
  
  kpss_trend_test <- tseries::kpss.test(df$value
                                        , null = "Trend"
                                        )
  
  kpss_level_df <- tibble::tibble(item = "nutrient - mineral"
                                  , group = nn
  ...
  kpss_trend_df <- tibble::tibble(item = "nutrient - mineral"
                                  , group = nn
