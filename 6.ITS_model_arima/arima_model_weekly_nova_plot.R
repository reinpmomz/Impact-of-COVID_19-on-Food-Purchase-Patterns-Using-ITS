...
## ITS plot
df_arima_ITS_model_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  
  df <- arima_model_all_weekly_nova[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(arima_model_interruption1_weekly_nova[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(arima_model_interruption2_weekly_nova[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
arima_ITS_model_plot_weekly_nova <- data.table::rbindlist(df_arima_ITS_model_weekly_nova) %>%
...
ggsave(plot=arima_ITS_model_plot_weekly_nova, height = 7.5, width = 13.5,
       filename = paste0("arima_ITS_plot_weekly_nova",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_arima_prediction_model_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  
  df <- arima_model_all_weekly_nova[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(arima_model_interruption1_weekly_nova[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(arima_model_interruption2_weekly_nova[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
arima_prediction_model_plot_weekly_nova <- data.table::rbindlist(df_arima_prediction_model_weekly_nova) %>%
...
ggsave(plot=arima_prediction_model_plot_weekly_nova, height = 7.5, width = 13.5,
       filename = paste0("arima_prediction_plot_weekly_nova",".png"),
       path = output_Dir, bg='white')
