...
## Period All
arima_model_all_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  ...
  ljung_test_df <- tibble::tibble(item = "nutrient - mineral"
                                  , group = nn
                                  , period = "all"
  ...
  ## Model metrics
  model_metrics <- dplyr::bind_cols(tibble::tibble( item = "nutrient - mineral"
                                                    , group = nn
                                                    , period = "all"
...
arima_model_interruption1_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  ...
  ## Model metrics
  model_metrics <- dplyr::bind_rows(tibble::tibble( item = "nutrient - mineral"
                                                    , group = nn
                                                    , period = "interruption 1"
                                                    , method = arima_forecast[["method"]]
                                                    , loglik = arima_forecast[["model"]][["loglik"]]
...
arima_model_interruption2_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
