...
## Period All
gls_model_all_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  
  df <- df_analysis_ITS_weekly_mineral %>%
  ...
  autocorrel_model <- sapply(1:nrow(autocorrel), function(x) {
    ...
    p <- try(summary(gls(value ~ weekly_time + intervention1 + post_intervention1_time +
  ...
  coefficients_df <- dplyr::bind_cols(lmtest::coeftest(gls_model#, df = summary_gls_model[["dims"]][["N"]]-1
    ...
    dplyr::mutate(item = "nutrient - mineral"
                  , group = nn) %>%
  ...
  ## Model metrics
  model_metrics <- tibble::tibble(item = "nutrient - mineral"
                                  , group = nn
                                  , period = "all"
...
gls_model_interruption1_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  ...
  autocorrel_model <- sapply(1:nrow(autocorrel), function(x) {
    ...
    p <- try(summary(gls(value ~ weekly_time
                         , data = df_train
                         , correlation= corARMA(p=autocorrel$pval[i],q=autocorrel$qval[i], form = ~ weekly_time)
                         ,method="ML")
                     )$AIC
...
gls_model_interruption2_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
...
## ITS plot
df_gls_ITS_model_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_mineral[[nn]][["predict_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_mineral[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_mineral[[nn]][["predict_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_ITS_model_plot_weekly_mineral <- data.table::rbindlist(df_gls_ITS_model_weekly_mineral) %>%
...
ggsave(plot=gls_ITS_model_plot_weekly_mineral, height = 7.5, width = 13.5,
       filename = paste0("gls_ITS_plot_weekly_mineral",".png"),
       path = output_Dir, bg='white')


## Prediction plot
df_gls_prediction_model_weekly_mineral <- sapply(unique(df_analysis_ITS_weekly_mineral$new_label), function(x) {
  nn <- x
  
  df <- gls_model_all_weekly_mineral[[nn]][["predict_fit_df"]] %>%
    dplyr::select(-period) %>%
    dplyr::left_join(gls_model_interruption1_weekly_mineral[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date")) %>%
    dplyr::left_join(gls_model_interruption2_weekly_mineral[[nn]][["predict_fit_df"]] %>%
                       dplyr::select(-period)
                     , by = c("group", "date"))
...
gls_prediction_model_plot_weekly_mineral <- data.table::rbindlist(df_gls_prediction_model_weekly_mineral) %>%
...
ggsave(plot=gls_prediction_model_plot_weekly_mineral, height = 7.5, width = 13.5,
       filename = paste0("gls_prediction_plot_weekly_mineral",".png"),
       path = output_Dir, bg='white')
