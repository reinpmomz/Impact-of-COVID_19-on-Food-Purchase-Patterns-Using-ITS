...
kpss_ITS_weekly_proximate <- sapply(unique(df_analysis_ITS_weekly_proximate$new_label), function(x) {
  nn <- x
  ...
  kpss_level_df <- tibble::tibble(item = "nutrient - proximate"
                                  , group = nn
  ...
  kpss_trend_df <- tibble::tibble(item = "nutrient - proximate"
                                  , group = nn
