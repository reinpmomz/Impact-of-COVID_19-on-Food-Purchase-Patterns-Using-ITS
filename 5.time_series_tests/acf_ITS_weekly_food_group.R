...
grDevices::png(filename = base::file.path(output_Dir, paste0("acf_ITS_weekly_food_group_plot", ".png")),
               height = 7.5,
               width = 12.5,
               units = "in",
               res = 300,
...
par(mfrow = c(4, 5), cex.main = 0.7)

acf_ITS_weekly_food_group <- sapply(unique(df_analysis_ITS_weekly_food_group$food_group), function(x) {
  nn <- x
  ...
  acf_test <- lmtest::dwtest(df$food_group_prop ~ df$weekly_time)
  ...
  ## Plot residuals
  acf_df <- acf(resid(lm(food_group_prop ~ weekly_time, data = df)
                        )
                  , type = "correlation"
                  , plot = FALSE
                  )
