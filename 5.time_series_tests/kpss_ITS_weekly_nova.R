...
kpss_ITS_weekly_nova <- sapply(unique(df_analysis_ITS_weekly_nova$nova), function(x) {
  nn <- x
  ...
  kpss_level_test <- tseries::kpss.test(df$nova_prop
                                        , null = "Level"
                                        )
  
  kpss_trend_test <- tseries::kpss.test(df$nova_prop
                                        , null = "Trend"
                                        )
  
  kpss_level_df <- tibble::tibble(item = "nova"
                                  , group = nn
  ...
  kpss_trend_df <- tibble::tibble(item = "nova"
                                  , group = nn
